                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <div class="small text-primary fw-medium">Luxury Branch</div>
                        </td>
                        <td>
                        ...
                        <td>
                            <span class="fw-semibold text-dark small">{{ $hotel->phone ?? '---' }}</span>
                        </td>
                        <td class="text-center">
                            <span class="badge-room">
                                <i class="fas fa-door-open me-1"></i> {{ $hotel->rooms_count }} phòng
                            </span>
                        </td>
                        <td class="pe-4">
                            <div class="action-btn-group">
                                <button class="btn-lux btn-lux-view" data-label="Chỉnh sửa">
                                    <i class="fas fa-pen-nib"></i>
                                </button>

                                <form action="{{ route('admin.hotels.destroy', $hotel->id) }}" method="POST" class="m-0">
                                    @csrf @method('DELETE')
                                    <button type="submit" class="btn-lux btn-lux-danger" data-label="Xóa hệ thống" 
                                            onclick="return confirm('Lưu ý: Xóa khách sạn sẽ xóa toàn bộ phòng thuộc khách sạn này. Tiếp tục?')">
                    ...
                    @empty
                    <tr>
                        <td colspan="6" class="text-center py-5 text-muted small">Không tìm thấy dữ liệu khách sạn.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
            ...
            <form action="{{ route('admin.hotels.store') }}" method="POST">
                ...
                <div class="modal-body p-4">
                    <div class="mb-4">
                        ...
                        <input type="text" name="name" class="form-control form-control-lux @error('name') is-invalid @enderror" 
                               placeholder="VD: Grand Plaza Hotel" value="{{ old('name') }}" required>
                    </div>
                    <div class="mb-4">
                        <label class="form-label fw-bold small text-uppercase text-muted">Địa chỉ chi tiết</label>
                        <textarea name="address" class="form-control form-control-lux" rows="2" placeholder="Số nhà, đường, quận, thành phố...">{{ old('address') }}</textarea>
                    </div>
                    <div class="mb-0">
                        <label class="form-label fw-bold small text-uppercase text-muted">Hotline liên hệ</label>
                        <input type="text" name="phone" class="form-control form-control-lux" placeholder="090xxx" value="{{ old('phone') }}">
                    </div>
                </div>
