                        ...
                        <div class="col-lg-9 col-md-8">
                            <div class="p-4 pe-5">
                                <div class="d-flex justify-content-between align-items-start mb-3">
                                    <div>
                                        <span class="text-uppercase small fw-bold text-muted letter-spacing-1">{{ $hotel?->name ?? 'Premium Hotel' }}</span>
                                        <h3 class="fw-bold mt-1 mb-2 text-dark">{{ $room?->name ?? 'Executive Suite' }}</h3>
                                        <p class="text-muted small mb-0">
                                            <i class="fas fa-map-marker-alt me-2 text-danger"></i>{{ $hotel?->address ?? 'Hà Nội, Việt Nam' }}
                                        </p>
                                    </div>
                                    <div class="text-end">
                                        <span class="badge-luxury {{ 
                                            ...
                                            {{ $booking->status == 'confirmed' ? 'Đã xác nhận' : ($booking->status == 'cancelled' ? 'Đã hủy' : 'Đang chờ') }}
                                        </span>
                                ...
                                <div class="row g-3 mb-4">
                                    ...
                                    <div class="col-md-4">
                                        <div class="info-block">
                                            ...
                                            <div class="info-value">{{ $booking->check_in->diffInDays($booking->check_out) }} Đêm</div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="d-flex justify-content-end align-items-center gap-4">
                                    @if($booking->status !== 'cancelled' && $booking->check_in->isFuture())
                                        <form action="{{ route('bookings.cancel', $booking) }}" method="POST" class="m-0">
                                            @csrf
                                            <button type="submit" class="btn-cancel-text border-0 bg-transparent p-0" onclick="return confirm('Hủy đặt phòng này?')">
                                                HỦY PHÒNG
                                            </button>
                                        </form>
                                    @endif
